  ...
  "ai": [
    {
      ...
      "temperature": 0.8,  // опционально: уровень креативности (0-2)
      "top_k": 40,         // опционально: лимит выбора токенов
      "top_p": 0.95,       // опционально: порог кумулятивной вероятности
      "format": "json",    // опционально: формат вывода (см. ниже)
      "timeout": 600000
    }
  ],
  "prompt": {
...
- `temperature`: (опционально) Управляет случайностью/креативностью ответов (0-2). Более высокие значения (например, 1.5) делают вывод более креативным, более низкие (например, 0.3) - более сфокусированным и детерминированным
- `top_k`: (опционально) Ограничивает выбор токенов топ-K наиболее вероятными вариантами (0-1000). Более высокие значения (например, 100) дают большее разнообразие, более низкие (например, 10) - более консервативны
- `top_p`: (опционально) Порог кумулятивной вероятности (0-1). Работает вместе с top_k. Более высокие значения (например, 0.95) приводят к более разнообразному тексту, более низкие (например, 0.5) генерируют более сфокусированный текст
- `format`: (опционально) Формат вывода для ответов AI. Подробности см. в [Ollama Structured Outputs](https://ollama.com/blog/structured-outputs). Три варианта:
  - `null` или не указан: без форматирования (по умолчанию)
  - `"json"`: базовое JSON форматирование - AI вернёт валидный JSON
  - JSON Schema объект: структурированный вывод с валидацией
    - **Пример объекта** (данные о человеке):
      ```jsonc
      {
        "type": "object",
        "properties": {
    ...
    - **Пример массива** (список задач):
      ...
      ```
  - **Совет**: Установите `temperature: 0` для детерминированного JSON вывода
